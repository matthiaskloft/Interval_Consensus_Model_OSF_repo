...
```{r}
sim_res_itm |>
  dplyr::select(contains("divtrans")) |>
  dplyr::select(-contains("mcse")) |> 
  rename(
...
```{r}
sim_res_itm |> 
  dplyr::select(all_of(contains("mcse"))) |> 
  dplyr::select(all_of(contains("bias"))) |> 
...
```{r}
sim_res_itm |> 
  dplyr::select(n_items, n_respondents, 
