...
subtitle: "Simulation Study: Development of Data Generating Process - Isometric Log-Ratio Transformaion"
author: 
 - name: Matthias Kloft
...
samples <- gather_values(
  lower = (round(responses$x_L, 2) + .01) / 1.03,
  upper = (round(responses$x_U, 2) + .02) / 1.03,
...
map_dfr(sim_data_list, 
  ...
  ggplot() +
  geom_histogram(
    aes(x = omega),binwidth = .02, fill = "purple", alpha = .7
  )+
  scale_x_continuous(
    limits = c(-1, 1),
    expand = expansion()
...
# save fit_ilr
fit_ilr$save_object(file =  here("fits", paste0(model_name, "_fit_ilr.rds")))
```

```{r}
# load  fit
fit_ilr <- readRDS(file =  here("fits", paste0(model_name, "_fit_ilr.rds")))
```
...
# save fit
fit_sb$save_object(file =  here("fits", paste0(model_name, "_fit_sb_cross.RDS")))
```

```{r}
# load  fit
fit_sb <- readRDS(file =  here("fits", paste0(model_name, "_fit_sb_cross.RDS")))
```
...
# join data
df_interval_plot_sb <- 
  full_join(latent_truth_est_sb, latent_truth_true) %>%
  full_join(means_sb) %>% 
  mutate(type = factor(type, levels = c("estimated", "true")))
```
...
#### Interval Plot
```{r message=FALSE, warning=FALSE}
cols <- c("True" = "grey70","ITM (SB - cross)" = "red", "Mean" = "blue")
...
df_interval_plot_sb %>%
  ...
  geom_errorbarh(
    aes(xmin = Tr_L_est, xmax = Tr_U_est, col = "ITM (SB - cross)"),
    height = 0,
    linewidth = 2,
    alpha = .5
    ) +
