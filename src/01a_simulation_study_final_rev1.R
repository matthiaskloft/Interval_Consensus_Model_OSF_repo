...
sim_analyze <- function(condition, dat, fixed_objects = NULL){
  ...
  simple_medians <- 
    ...
    as.data.frame() |>
    dplyr::group_by(jj) |>
    dplyr::summarise(across(everything(), stats::median, na.rm = TRUE)) |>
    dplyr::ungroup() |>
    dplyr::select(-jj) |>
    rename(simplemed_loc = x_bvn_1, simplemed_wid = x_bvn_2)
  ...
  return(
    ret = list(
      ...
      true_responses = responses,
      simple_means = simple_means,
      simple_medians = simple_medians
    )
  )
}
...
sim_summarize <- function(condition, results, fixed_objects = NULL){
  ...
  # sometimes use old pipe for easier handling of "." 
  library(magrittr)
  library(dplyr)
  ...
  # Method can either be "model" for the ITM model or 
  # or "means" for the simple means
  # or "medians" for the simple medians
  point_comparison <- function(results = results,
                               ...
                               summary,
                               pm,
                               transform_fn = NULL) {  # transforming values, e.g. log
  ...
  ret_pm_values <- vector("list", length(results)) 
  for (i in 1:length(results)) { 
    current_est_val <- tmp_summary[[i]]
    if(!is.null(transform_fn)) {
      current_est_val <- transform_fn(current_est_val)
    }
    true_val <- results[[i]]$true_parameters[[truth_name]]
    if(!is.null(transform_fn)) {
      true_val <- transform_fn(true_val)
    }
    ret_pm_values[[i]] <- pm(current_est_val, true_val)

  }
  return(ret_pm_values) 
}
  ...
  fn_corr <- function(est_param, 
                     true_param, 
                     average = TRUE) {
    if (isTRUE(average)) {
      cor(est_param, true_param, use = "pairwise.complete.obs")
    } else {
      cor(est_param, true_param, use = "pairwise.complete.obs")
    }
  }
  ...
  performance_results <- performance_grid %>%
    dplyr::mutate(raw = purrr::pmap(., function(estimate_name, method, truth_name, summary, pm) {
      point_comparison(
        results = results,
        ...
        transform_fn = NULL # no transformation for most results
        )
    }))
  
  
  # Add new log-transformed parameters
  log_est_names <- c("a_loc", "E_loc", "E_wid", "lambda_loc", "lambda_wid")
  
  log_grid <- expand.grid(
    estimate_name = log_est_names,
    summary = c("median", "mean"),
    pm = c("fn_abs_bias", "fn_mse", "fn_corr"),
    stringsAsFactors = FALSE
  )
  
  log_grid$truth_name <- log_grid$estimate_name

  performance_results_log_df <- log_grid %>%
    dplyr::mutate(
      raw = purrr::pmap(.,
        .f = function(estimate_name, method, truth_name, summary, pm) {
          point_comparison(
            results = results,
            estimate_name = estimate_name, 
            method = "model",           
    ...
    dplyr::mutate(
      estimate_name = paste0(estimate_name, "_log"),
      truth_name = estimate_name
    )

  # append results
  if (nrow(performance_results_log_df) > 0) {
    performance_results <- dplyr::bind_rows(performance_results, performance_results_log_df)
  }

  
  # Evaluate simple means and medians
  methods <- c("means", "medians")
  estimates <- c("simplemean_loc", "simplemean_wid", "simplemed_loc", "simplemed_wid")
  pms <- c("fn_abs_bias", "fn_mse")
  
  for (method in methods) {
    for (estimate in estimates) {
      # break the loop if method and estimate are not compatible
      if (grepl("mean", estimate) & method == "medians") {
        next
      }
      if (grepl("med", estimate) & method == "means") {
        next
      }
      
      for (pm in pms) {
        truth_name <- ifelse(grepl("loc", estimate), "Tr_loc", "Tr_wid")
  ...
  # method can be either "model",  "means" or "medians" 
  interval_comparison <- function(results = results,
    ...
    loc_summary <- sapply(results, function(x) {
      ...
      if (method == "model") {
        ...
        x$simple_medians |>
          dplyr::select(all_of(contains("simplemed_loc")))
      }
    ...
    wid_summary <- sapply(results, function(x) {
      if (method == "model") {
        ...
        x$simple_medians |>
          dplyr::select(all_of(contains("simplemed_wid")))
      }
    })
  ...
  performance_results_int <- performance_results |>
    ...
    # add simple median intervals
    tibble::add_row(
      estimate_name = "simplemed_interval",
      summary = "mean",
      pm = "fn_abs_bias",
      truth_name = "Tr_interval",
      raw = list(
...
# n_rep <- 2
# sim_pars$n_chains <- 2
# sim_pars$iter_warmup <- 200
# sim_pars$iter_sampling <- 500
# n_cores <- n_rep
...
#' # Resummarize
#' 
#' With an adapted summarize function, we can resummarize the results of the simulation study to include measures on the log-scale. 
## -----------------------------------------------------------------------------------
# Resummarize
resummarized_results <- SimDesign::reSummarise(
  summarise = sim_summarize, 
  dir = here::here("sim_results", "sim_results_itm_2025-04-28_03-30-27", "full_sim_results_itm"),
)

saveRDS(resummarized_results, file = here("sim_results", "sim_results_itm_2025-04-28_03-30-27", "resummarized_results_itm.rds"))
...
#' 
## -----------------------------------------------------------------------------------
# knitr::purl(here("src", "01a_simulation_study_final_rev1.Rmd"),
#             output = here("src", "01a_simulation_study_final_rev1.R"),
#             documentation = 2)
