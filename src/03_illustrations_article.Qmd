...
plot_2d_scatter_list <-
  map(
    ...
    if (.x %in% c("proficiency width", "shifting bias width")) {
      plot_example_2Dscatter(data)
    } else {
      # ommit x-axis label for other conditions
      plot_example_2Dscatter(data) +
...
```{r}
# plot_intervals_list[[2]] <- 
#   plot_intervals_list[[2]] +
#   geom_segment(aes(x = .5, xend = .9, y = 4, yend = 4),
#     arrow = arrow(),
#     color = "red",
#     size = 1, 
#     linewidth = .1
...
ggsave(
  plot = plot_combined, 
  filename = here("plots", "icm_parameter_illustration.pdf"),
  width = 15,
  height = 11,
  units = "cm",
  scale = 1.8
